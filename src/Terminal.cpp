...
template ColorTerminal<true>::ColorTerminal(
	const Color& color,
	const Color& background_color);

template ColorTerminal<false>::ColorTerminal(
	const Color& color,
	const Color& background_color);
...
void ColorTerminal<F>::Reset()
{
	...
	NumberOfColumns = windowSize.ws_col / 2;

	ScreenBuffer = std::vector(
...
template <bool F>
void ColorTerminal<F>::Draw(
	unsigned x,
	unsigned y,
	const char *mchar,
	int colorShade)
{
	// Convert Matrix logical column to terminal column.
...
template <bool F>
void ColorTerminal<F>::DrawTitle(
	unsigned x,
	unsigned y,
	wchar_t tchar)
{
...
{
	std::cout.write(
		reinterpret_cast<char*>(ScreenBuffer.data()),
